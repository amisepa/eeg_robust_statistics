...
[mask, pcorr] = compute_mcc(tvals, pvals, tvals_H0, pvals_H0, corr_method, alphalvl, neighbormatrix);
...
%% Perform GLM
% Requires to have a folder with subject files with trial data
...
optimization = 'OLS';   % 'OLS' 'IRLS' or 'WLS'
...
for iSub = 1:nSub
    ...
    filepath = fullfile(data_dir,filenames{iSub});
    ...
    PLEASANT(:,:,iSub) = mean(EEG.data(:,:,idx),3,'omitnan');       % for comparison
    idx = strcmp({EEG.event.type},'4');
    % NEUTRAL(:,:,iSub) =  trimmean(EEG.data(:,:,idx),20,3);        % for comparison
    NEUTRAL(:,:,iSub) = mean(EEG.data(:,:,idx),3,'omitnan');        % for comparison
    idx = strcmp({EEG.event.type},'8');
    % UNPLEASANT(:,:,iSub) =  trimmean(EEG.data(:,:,idx),20,3);     % for comparison
    UNPLEASANT(:,:,iSub) = mean(EEG.data(:,:,idx),3,'omitnan');     % for comparison
...
if strcmpi(optimization,'WLS')
    print(gcf, fullfile(out_dir,sprintf('result_unpleasant-neutral_GLM_%s-%s_corrected.png',optimization,weightmethod)),'-dpng','-r300');   % 300 dpi .png
else
    print(gcf, fullfile(out_dir,sprintf('result_unpleasant-neutral_GLM_%s_corrected.png',optimization)),'-dpng','-r300');   % 300 dpi .png
end
...
if strcmpi(optimization,'WLS')
    print(gcf, fullfile(out_dir,sprintf('result_unpleasant-neutral_GLM_%s-%s_corrected_peak-channel.png',optimization,weightmethod)),'-dpng','-r300');   % 300 dpi .png
else
    print(gcf, fullfile(out_dir,sprintf('result_unpleasant-neutral_GLM_%s_corrected_peak-channel.png',optimization)),'-dpng','-r300');   % 300 dpi .png
end
...
[mask, pcorr, nClust] = compute_mcc(tvals, pvals, tvals_H0, pvals_H0, corr, alpha_lvl, neighbormatrix);
[peakClust, peakChan, peakLat, clustOrder] = plot_results('time', times, tvals, mask, pcorr, alpha_lvl, chanlocs, 2); 
print(gcf, fullfile(out_dir,'result_unpleasant-neutral_RAW_corrected.png'),'-dpng','-r300');   % 300 dpi .png
...
print(gcf, fullfile(out_dir,'result_unpleasant-neutral_ERP_corrected_peak-channel.png'),'-dpng','-r300');   % 300 dpi .png
